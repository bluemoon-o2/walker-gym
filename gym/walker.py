...
class Muscle:
    ...
    def run(self):
        ...
        self.p1.anti_forced(dk * self.dampk, self.p2.p)
        self.p2.anti_forced(dk * self.dampk, self.p1.p)
...
class Creature:
    ...
    def getstat(self,in3d=True,pk=1,vk=1,ak=1,mk=1,midform=True,conmid=False):
        ...
        if midform:
            ...
            mid[2]/len(self.phys)
        for i in self.phys:
            s+=[(i.p[j]-mid[j])*pk for j in range(d)]+[j*vk for j in i.v[:d]]+[ac * ak for ac in i.old_a[:d]]
        if conmid:
            s+=mid
        for i in self.muscles:
            s.append(i.x*mk)
